                                                            ...
                                                            <div class="course-item-content-wrap">
                                                                ...
                                                                <div class="course-item-content">
                                                                    <h4 class="fs-15 lecture-title"
                                                                        ...
                                                                        data-content="{{ htmlspecialchars($lect->content ?? '', ENT_QUOTES, 'UTF-8') }}">
    ...
    <script type="text/javascript">
        ...
        function convertToEmbedUrl(url) {
            ...
            if (url.includes('youtube.com/watch')) {
                try {
                    // Parse URL to extract video ID and other parameters
                    const urlObj = new URL(url);
                    const videoId = urlObj.searchParams.get('v');
                    const timeParam = urlObj.searchParams.get('time_continue') || urlObj.searchParams.get('t');
            ...
            if (url.includes('vimeo.com/')) {
                ...
                const cleanVideoId = videoId ? videoId.split('?')[0] : videoId;
                return cleanVideoId ? `https://player.vimeo.com/video/${cleanVideoId}` : url;
            }
        ...
        function viewLesson(videoUrl, vimeoUrl, textContent) {
            ...
            if (videoUrl && videoUrl.trim() !== "") {
                ...
                text.innerHTML = "";
                // Convert to embed URL before setting src
                ...
                text.innerHTML = "";
                // Convert to embed URL before setting src
                const embedUrl = convertToEmbedUrl(vimeoUrl);
                console.log("Converted Vimeo URL:", embedUrl);
                video.setAttribute("src", embedUrl);
            } else if (textContent && textContent.trim() !== "") {
                video.classList.add("d-none");
                text.classList.remove("d-none");
                text.innerHTML = "";
                // Decode HTML entities since we escaped them in the blade template
                const decodedContent = decodeHtmlEntities(textContent);
                textContainer.innerHTML = decodedContent;
        ...
        document.querySelectorAll('.lecture-title').forEach((lectureTitle) => {
            lectureTitle.addEventListener('click', () => {
                ...
                // Since we only have one URL field, we'll determine if it's YouTube or Vimeo
                if (videoUrl && (videoUrl.includes('youtube.com') || videoUrl.includes('youtu.be'))) {
                    viewLesson(videoUrl, null, textContent);
                } else if (videoUrl && videoUrl.includes('vimeo.com')) {
                    viewLesson(null, videoUrl, textContent);
                } else {
                    viewLesson(videoUrl, null, textContent);
                }
            });
